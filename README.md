...
- `Логи` поддерживают серверную фильтрацию по источнику и диапазону `from/to` времени, а также локальный поиск по тексту.
- `ML-модуль` для anomaly detector показывает не только `ready`, но и понятную причину текущего состояния: ожидание, обучение, недостаточно данных, отложено, ошибка.
